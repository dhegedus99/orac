...
subroutine interpolate_meteorology(lon,lat,nlev,xdim,ydim, &
   ...
   ! Check if the point is outside the bounds (considering wrap)
   if (Wrap) then
       OutOfBounds = .false.  ! If wrapping, we always interpolate
   else
       OutOfBounds = ilon < MinLon .or. ilon > MaxLon .or. ilat < min(Lat0,LatN) .or. ilat > max(Lat0,LatN)
   end if

   if (OutOfBounds) then
       ! Assign missing value if out of bounds
       iP  = sreal_fill_value
       iT  = sreal_fill_value
       iH  = sreal_fill_value
       iQ  = sreal_fill_value
       iO3 = sreal_fill_value
   else
